...
namespace HotelManager
{
    public partial class fUseService : Form
    {
        ...
        {
            if (MetroFramework.MetroMessageBox.Show(this, "Bạn có chắc chắn thanh toán cho " + "không?", "Thông báo", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                Room room = flowLayoutRooms.Tag as Room;
                int idBill = BillDAO.Instance.GetIdBillFromIdRoom(room.Id);
                Pay(idBill,int.Parse(numericUpDown1.Value.ToString()));
                MetroFramework.MetroMessageBox.Show(this, "Thanh toán thành công!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                this.Hide();
